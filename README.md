...
**Key Technical Focus:** I implemented a Moving Average algorithm utilizing a Rolling Buffer (deque) data structure. This manages the trade-off between responsiveness and stability, ensuring the system operates on clean, smoothed data.
...
**Key Technical Focus:** I transitioned to a multi-tier architecture with a secure REST API. It features Non-blocking I/O for high-concurrency and secure credential management via external configuration files.
...
2. **Run command:** Execute the compiled binary:  
   `./ThermostatSystem`
